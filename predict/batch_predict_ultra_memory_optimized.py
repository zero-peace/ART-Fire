...
def force_memory_cleanup():
    ...
    # 显示内存状态
    print(f"  📊 当前内存状态: {get_memory_info()}")

class UltraMemoryOptimizedBatchPredictor:
    def __init__(self, model_path='Maple728/TimeMoE-200M', device='auto'):
        """
        超内存优化的批量预测器
    ...
    def _load_model(self, model_path):
        ...
        try:
            ...
            # 使用CPU加载模型以减少GPU内存使用
            temp_device = 'cpu' if self.device == 'cuda' else self.device
            self.predictor = TimeSeriesPredictorV2(model_path=model_path, device=temp_device)
            
            # 如果目标设备是GPU，将模型移到GPU
            if self.device == 'cuda':
                print("🔄 将模型移动到GPU...")
                self.predictor.model = self.predictor.model.to('cuda')
                force_memory_cleanup()
    ...
    def predict_single_file_ultra_optimized(self, csv_path, bands, start_timestamp, end_timestamp, 
    ...
    def run_ultra_optimized_batch_prediction(self, folder_path, bands, start_timestamp, end_timestamp,
                                           use_len=144, prediction_steps=1, timestamp_col='timestamp',
                                           base_output_dir='./ultra_memory_optimized_predictions',
        ...
        for batch_start in range(0, total_files, batch_size):
            ...
            for i, csv_path in enumerate(batch_files):
                ...
                for retry in range(max_retries):
                    try:
                        ...
                        # 预测单个文件
                        result = self.predict_single_file_ultra_optimized(
                            csv_path=csv_path,
                            bands=bands,
                            start_timestamp=start_timestamp,
                            end_timestamp=end_timestamp,
            ...
            # 批次处理完成后清理内存
            print(f"\n🧹 批次 {batch_start//batch_size + 1} 完成，执行深度内存清理...")
            force_memory_cleanup()
        ...
        summary_file = os.path.join(base_output_dir, 'ultra_memory_optimized_summary.csv')
...
def main():
    ...
    batch_size_input = input("📦 批处理大小 (默认1，按回车使用默认值): ").strip()
    batch_size = int(batch_size_input) if batch_size_input else 1
    
    max_retries_input = input("🔄 最大重试次数 (默认3，按回车使用默认值): ").strip()
    ...
    # 创建超内存优化预测器
    predictor = UltraMemoryOptimizedBatchPredictor(device=device)
    
    # 运行预测
    results = predictor.run_ultra_optimized_batch_prediction(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        # 命令行模式
        if len(sys.argv) < 5:
            print("用法: python batch_predict_ultra_memory_optimized.py <文件夹路径> <波段1 波段2...> <开始时间> <结束时间> [使用长度] [预测步数] [设备] [批处理大小] [重试次数]")
            print("示例: python batch_predict_ultra_memory_optimized.py './data' 'albedo_01 tbb_07' '2022-10-15 01:00:00' '2022-10-16 01:00:00' 144 1 cuda 1 3")
            sys.exit(1)
        ...
        max_retries = int(sys.argv[9]) if len(sys.argv) > 9 else 3
